...
namespace StoreHouse360.Application.Commands.Journals
{
    ...
    public class CreateJournalsCommandHandler : IRequestHandler<CreateJournalsCommand, (int DebitId, int CreditId)>
    {
        ...
        public async Task<(int DebitId, int CreditId)> Handle(CreateJournalsCommand request, CancellationToken cancellationToken)
        {
            var (debit, credit) = new JournalsFactory().CreateJournals(request.SourceAccountId, request.DestinationAccountId, request.Value, request.CurrencyId);
