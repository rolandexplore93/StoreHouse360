namespace StoreHouse360.Application.Common.Security
{
    public static class PolicyHelper
    {
        ...
        {
            if (!IsValidPolicy(policy)) throw new InvalidDataException($"Invalid policy string {policy}");
        ...
        {
            return policies.Split(PoliciesSeparator).Select(CreatePolicyFromString);
        }

        public static bool IsValidPolicy(string policy)
        {
            // Split policy input into an array by a comma
            var policies = policy.Split(PoliciesSeparator);
    ...
    internal static class MethodHelper
    {
        ...
        {
            var methods = Enum.GetValues<Method>().ToList();
    ...
    internal static class ResourceHelper
    {
        ...
        {
            var resources = Enum.GetValues<Resource>().ToList();
